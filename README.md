...
All `builtin.*` tools are grantable. `builtin.web_search` and
`builtin.weather_lookup` are in the default tool grants so agents can satisfy
ordinary public lookup requests; operators grant access to other built-ins
explicitly via `tools.grant`.
